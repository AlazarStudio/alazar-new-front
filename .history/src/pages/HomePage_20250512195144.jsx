...
const HomePage = () => {
  ...
  useEffect(() => {
    Promise.all([
      fetch('https://backend.alazarstudio.ru/api/cases').then((res) =>
        res.json()
      ),
      fetch('https://backend.alazarstudio.ru/api/categories').then((res) =>
        res.json()
      ),
      fetch('https://backend.alazarstudio.ru/api/developers').then((res) =>
        res.json()
      ),
  ...
  const pinnedCases = filteredCases.filter((c) => c.positionTop);
  ...
  const pseudoRandom = (id) => {
    ...
    return Math.abs(hash % 10000); // любое небольшое число
  };
  ...
  return (
    <Box className={styles.container}>
      ...
      <Box className={styles.casesWrapper}>
        {filteredCases
          .sort((a, b) => (b.positionTop ? 1 : 0) - (a.positionTop ? 1 : 0))

          
            [...pinnedCases, ...unpinnedCases].map((c) => (
              <Box
                key={c.id}
