...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    if (categorySlug && categories.length) {
        const foundCategory = categories.find((cat) => cat.name === categorySlug);
        if (foundCategory) {
          setActiveCategoryId(foundCategory.id);
        }
      } else {
        setActiveCategoryId(null); // <-- вот это важно!
      }
  ...
  const handleCategorySelect = (id) => {
    ...
    if (id === null) {
      ...
      if (category) {
        currentParams.set('category', category.name); // или .slug
      }
    }
  
    // сохранить кейс если открыт
    if (selectedCase) {
      currentParams.set('case', selectedCase.id);
    }
